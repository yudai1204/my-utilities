chrome.storage.local.get(
  ...
  function (data) {
    for (const item of document.querySelectorAll(".autoitem")) {
      item.value = data.data[item.getAttribute("id")]
        ? data.data[item.getAttribute("id")]
        : "";
    }
    for (const item of document.querySelectorAll(".settingitem")) {
      item.checked = data.setting[item.getAttribute("id")]
        ? data.setting[item.getAttribute("id")]
        : false;
    }
    document.getElementById("passjson").value = JSON.stringify(
      data.googlePasswords,
      null,
      "\t"
    );
  }
...
function create_csv() {
  ...
  //ファイル名の指定
  const now = new Date();
  let file_name = `formAutoInput_${
    now.getMonth() + 1
  }_${now.getDate()}_${now.getHours()}_${now.getMinutes()}.csv`;
...
function create_json() {
  ...
  let file_name = `formAutoInput_${
    now.getMonth() + 1
  }_${now.getDate()}_${now.getHours()}_${now.getMinutes()}.json`;
